...
def train_model():
    ...
    # 【追加】訓練NME記録用のリスト
    train_nmes = []
    # 【既存】テストNMEは記録しないが、ここでは便宜上リストを準備
    test_nmes = [] # NMEプロット用に、最終的にはこちらもリスト化しても良い
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # ⚠️ 注: avg_train_loss と train_loss_epoch は、optimizer.step() の有無により僅かに異なる可能性あり
        
        # --- テストデータでの評価 ---
        test_loss, test_nme = evaluate_model(model, test_loader, criterion, device)
        test_losses.append(test_loss)
        test_nmes.append(test_nme) # NMEを記録
