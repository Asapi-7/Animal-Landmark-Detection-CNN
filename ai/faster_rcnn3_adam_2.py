...
# モデル構築用
from resnet18_fpn import resnet18_fpn
...
# さらにtempを検証用(50%) とテスト用(50%) に分割（今回はテスト用のみ使用）
...
val_dataset = CustomObjectDetectionDataset(val_imgs, DATA_ROOT, train=False)
...
# モデル構築
# ==========================
backbone = resnet18_fpn(pretrained=False)
...
optimizer = optim.Adam(
    model.parameters(),
    lr=0.0001,         # ← AdamはSGDよりも小さめが安定（例: 1e-4〜1e-5）
    weight_decay=0.0005
)
...
for epoch in range(num_epochs):
    ...
    if(epoch + 1) %5 == 0 or (epoch +1) == num_epochs:
        ...
        save_path = f"fasterrcnn_resnet18_adam_{epoch+1}.pth"
...
# 最終評価
# ==========================
evaluate_fasterRCNN(model, val_loader, device)
