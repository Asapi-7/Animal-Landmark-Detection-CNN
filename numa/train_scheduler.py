...
def train_model():
    ...
    if len(all_files) == 0:
        raise FileNotFoundError("データセット内に .jpg がありません")

    print(f"全画像数: {len(all_files)}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # scheduler に validation loss を渡す
        scheduler.step(valid_loss)

        with open(LOG_FILE, "a") as f:
            f.write(f"{epoch+1},{avg_train_loss:.6f},{avg_train_nme:.6f},"
                    f"{valid_loss:.6f},{valid_nme:.6f}\n")

        print(f"[Epoch {epoch+1}]  "
              f"Train MSE: {avg_train_loss:.4f}, Train NME: {avg_train_nme:.4f},  "
    ...
    # モデル保存
    torch.save(model.state_dict(), "model_scheduler.pth")
    print("\nモデル保存完了: model_scheduler.pth")
...
if __name__ == "__main__":
    ...
    save_landmark_predictions(
        ...
        device=device,
        num_samples=5,
        save_dir="./predictions_train_sceduler"  # 保存フォルダは自由に変更
    )
