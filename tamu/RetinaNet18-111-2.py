...
class CustomObjectDetectionDataset(Dataset): # DAtasetクラスを継承
    ...
    def __init__(self, img_list, root, transforms=None, augment=False): 
        ...
        self.augment_transform = A.Compose([ # データ拡張
            ...
            A.Affine(translate_percent={'x': (-0.02, 0.02), 'y': (-0.02, 0.02)}, scale=(0.95, 1.05),rotate=(-5, 5),border_mode=0, p=0.4),
            ToTensorV2()
        ], 
...
# モデルの重みを保存
torch.save(model.state_dict(), 'retinanet181112_weights_SGD.pth')
