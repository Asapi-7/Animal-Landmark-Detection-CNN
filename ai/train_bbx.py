...
class AnimalWebDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        bb = np.array([coords[0][0], coords[0][1], coords[1][0], coords[1][1]], dtype=np.float32)

        # --- 正規化 ---
        img = torch.from_numpy(img).permute(2, 0, 1).float() / 255.0
        bb = torch.from_numpy(bb).float() / torch.tensor([W, H, W, H], dtype=torch.float32)
